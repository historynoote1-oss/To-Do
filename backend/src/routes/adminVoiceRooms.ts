...
router.post('/', async (req: AuthRequest, res) => {
  ...
  const maxMembersRaw = req.body?.maxMembers;
  const maxMembers =
    maxMembersRaw === null || maxMembersRaw === undefined || maxMembersRaw === ''
      ? null
      : Number.isFinite(Number(maxMembersRaw)) && Number(maxMembersRaw) > 0
        ? Math.floor(Number(maxMembersRaw))
        : undefined;
  ...
  if (maxMembers === undefined) return res.status(400).json({ error: 'الحد الأقصى للأعضاء غلط' });

  const room = await prisma.voiceRoom.create({
    data: { name, description: description || null, createdById: req.userId, maxMembers },
  });
  res.status(201).json({
    room: {
      id: room.id,
...
router.patch('/:roomId', async (req: AuthRequest, res) => {
  ...
  const data: { name?: string; description?: string | null; maxMembers?: number | null } = {};
  ...
  if ('maxMembers' in (req.body || {})) {
    const raw = req.body.maxMembers;
    if (raw === null || raw === '') {
      data.maxMembers = null;
    } else if (Number.isFinite(Number(raw)) && Number(raw) > 0) {
      data.maxMembers = Math.floor(Number(raw));
    } else {
      return res.status(400).json({ error: 'الحد الأقصى للأعضاء غلط' });
    }
  }

  const updated = await prisma.voiceRoom.update({ where: { id: room.id }, data });
  res.json({ room: { id: updated.id, name: updated.name, description: updated.description || null, maxMembers: updated.maxMembers } });
});
