...
export async function createAdminVoiceRoom(
  name: string,
  description?: string,
  maxMembers?: number | null,
): Promise<AdminVoiceRoomEntry> {
  const res = await fetch(`${API_URL}/api/admin/voice-rooms`, {
    method: 'POST',
    headers: authHeaders(),
    body: JSON.stringify({ name, description, maxMembers: maxMembers ?? null }),
  });
  const data = await handle(res);
  return data.room;
}
...
export async function updateAdminVoiceRoom(
  roomId: string,
  patch: { name?: string; description?: string; maxMembers?: number | null },
): Promise<{ id: string; name: string; description: string | null; maxMembers: number | null }> {
