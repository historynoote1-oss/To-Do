...
type SoundsState = { muted: boolean; volume: number };
const soundsStore = createListenerSet<SoundsState>();

function emitChange() {
  soundsStore.emit({ muted: isMuted(), volume: getVolume() });
}
...
export const sounds = {
  ...
  subscribe(listener: (state: SoundsState) => void) {
    return soundsStore.subscribe(listener);
  },
  ...
  // ===== أصوات مؤقت البومودورو — مصممة عمدًا أعلى وأوضح من كل الأصوات
  // التانية في الملف، لأن الهدف هنا مختلف: المستخدم مركّز/مذاكر ومش
  // شايف الشاشة أصلًا، فالصوت لازم يلفت انتباهه فعليًا مش مجرد "تنبيه
  // خفيف". لسه بيحترم إعداد الكتم/مستوى الصوت العام زي أي صوت تاني
  // (بيعدي على نفس masterGain)، لكن الـ gainPeak وعدد النغمات المتكررة
  // أعلى بوضوح من باقي دوال الملف. =====

  // "متبقي دقيقة" — ثلاث نقرات متتالية واضحة وحادة (مش نقرتين خفيفتين زي
  // timelineWarning)، بتتشغّل مرة واحدة بس لحظة ما العداد يعدّي عتبة
  // التحذير عشان تلفت الانتباه من غير ما تبقى مصدر إزعاج متكرر.
  pomodoroWarning() {
    for (let i = 0; i < 3; i++) {
      tone({ freq: 880, duration: 0.1, type: 'triangle', gainPeak: 0.17, delay: i * 0.16, filterFreq: 4200 });
    }
  },

  // "خلص الوقت" — جرس منبّه حقيقي: خمس رنّات متبادلة بين نغمتين (زي منبّه
  // ساعة فعلي) بموجة مربّعة (square) عشان تبقى حادة وواضحة تخترق أي ضوضاء
  // حواليك، وبـ gainPeak أعلى من أي صوت تاني في التطبيق بالكامل.
  pomodoroEnd() {
    const beeps = 5;
