...
export default function VoiceRoomMemberTile({
  ...
  return (
    <div className="voice-room-tile" style={{ background: `linear-gradient(155deg, ${bg}55, ${bg}22)` }}>
      <span className="voice-room-tile-avatar">
        {url ? <img src={url} alt="" /> : <span aria-hidden="true">{isBot ? '📖' : initial}</span>}
      </span>
      <span className="voice-room-tile-name-pill">
        {member.isAdmin && !isBot && <DynamicIcon name="crown" size={12} className="voice-room-tile-crown" />}
        {isBot && <DynamicIcon name="radio" size={12} className="voice-room-tile-crown" />}
