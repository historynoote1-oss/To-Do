...
import AccountPasswordConfirmModal from '@/pages/auth/AccountPasswordConfirmModal';
import { DynamicIcon } from '@/utils/icons';
import { sounds } from '@/services/audio/sounds';
import { CategoryKey, categoryOf, MONTH_NAMES, WEEK_LABELS, DAY_OF_WEEK_NAMES } from '@/utils/category';
import { LifeAreaData, hexToSoftBg } from '@/utils/lifeArea';
import type { NewTaskPayload } from '@/pages/tasks/AddTaskModal';
import type { GoalOption, TrashedYear } from '@/services/api';
import {
  getTrash,
...
const AddTaskModal = lazy(() => import('@/pages/tasks/AddTaskModal'));

import { GoalList, GoalMapProps as Props } from './goalMapTypes';
import { CURRENT_YEAR, LEVELS, LONG_PRESS_MS, MAX_YEAR, clampToFutureYear, pluralIndefinite } from './goalMapHelpers';
import ZoomFolderGrid from './ZoomFolderGrid';
...
export default function GoalMap({
  ...
  async function refreshTrash() {
    ...
    try {
      const trashedYearsResponse = await getTrash();
      setTrashedYears(trashedYearsResponse);
    } catch {
  ...
  // الحساب قبل التنفيذ الفعلي، شوف services/api.ts (deleteListCascade) و
  ...
  async function confirmDeleteYearNow() {
    const yearToDelete = confirmDeleteYear;
    ...
    const hasRealGoals = yearlyGoals.some((g) => (g.targetYear || CURRENT_YEAR) === yearToDelete);
    if (!hasRealGoals) {
      persistExtraYears(extraYears.filter((ey) => ey !== yearToDelete));
      toast.success(`سنة ${yearToDelete} اتشالت`);
    ...
    try {
      await trashYearApi(yearToDelete);
      toast.success(`سنة ${yearToDelete} اتنقلت لسلة المحذوفات — تقدر تسترجعها خلال 5 أيام`);
      setConfirmDeleteYear(null);
      persistExtraYears(extraYears.filter((ey) => ey !== yearToDelete));
      onChange();
      refreshTrash();
  ...
  return (
    <div className="container view-fade profile-page goal-map-page">
          ...
          {trashedYears.length > 0 && (
            <div className="notifications-bell-wrap">
              <button
                ...
                type="button"
                title="سلة محذوفات السنين"
                aria-label={`سلة محذوفات السنين (${trashedYears.length})`}
              >
      ...
      {trashPanelOpen && (
        <Portal>
          <div className="modal-overlay" onClick={() => setTrashPanelOpen(false)}>
            <div className="modal-box" onClick={(e) => e.stopPropagation()}>
              <h2>سلة محذوفات السنين</h2>
              <div className="modal-text">
                السنين هنا اتحذفت وهتتشال نهائيًا تلقائيًا بعد انتهاء الأيام الموضّحة تحت كل واحدة، إلا لو استرجعتها قبل كده.
              </div>

              {trashLoading && trashedYears.length === 0 && <p className="empty small">بيتحمّل...</p>}

              {!trashLoading && trashedYears.length === 0 && <p className="empty small">سلة المحذوفات فاضية.</p>}

              {trashedYears.length > 0 && (
                <ul className="trash-years-list">
                  {trashedYears.map((t) => (
                    <li key={t.year} className="trash-years-item">
                      <div>
                        <strong>{t.year}</strong>
                        <div className="trash-years-item-meta">
                          {t.totalGoals} هدف · باقي {t.daysLeft} {t.daysLeft === 1 ? 'يوم' : 'أيام'}
                        </div>
                      </div>
                      <button
                        className="small"
                      ...
                      >
                        {restoringYear === t.year ? 'بيسترجع...' : 'استرجاع'}
                      </button>
                    </li>
                  ))}
                </ul>
              ...
              <div className="modal-actions">
                <button className="small" onClick={() => setTrashPanelOpen(false)} type="button" autoFocus>
                  إغلاق
                </button>
              </div>
            </div>
      ...
      {/* ===== المرحلة الثانية: تأكيد أخير بكلمة مرور الحساب قبل التنفيذ
          الفعلي — شوف AccountPasswordConfirmModal.tsx وservices/api.ts
          (deleteListCascade). ===== */}
      {cascadePasswordFor && (
