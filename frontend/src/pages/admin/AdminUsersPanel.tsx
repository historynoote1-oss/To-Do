...
import AdminConfirmModal from '@/pages/admin/AdminConfirmModal';
...
export default function AdminUsersPanel() {
  ...
  async function loadUsers(page: number, q: string) {
    ...
    try {
      const usersResponse = await getAdminUsers({ q, page, pageSize: 20 });
      setUsers(usersResponse.users);
      setUserTotalPages(usersResponse.totalPages);
  ...
  async function loadLogs(page: number) {
    setLogsLoading(true);
    try {
      const auditLogResponse = await getAdminAuditLog({
        page,
        pageSize: 50,
        adminUsername: logAdminFilter || undefined,
        action: logActionFilter || undefined,
      });
      setLogs(auditLogResponse.logs);
      setLogTotalPages(auditLogResponse.totalPages);
      setLogTotal(auditLogResponse.total);
      setAvailableActions(auditLogResponse.availableActions);
    } catch (err) {
