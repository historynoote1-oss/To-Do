...
function ListsManager() {
  ...
  async function load(p: number, query: string, statusFilter: typeof status) {
    setLoading(true);
    try {
      const listsResponse = await getAdminLists({ q: query, status: statusFilter, page: p, pageSize: 20 });
      setLists(listsResponse.lists);
      setTotalPages(listsResponse.totalPages);
      setTotal(listsResponse.total);
    } catch (err) {
...
function ItemsManager() {
  ...
  async function load(p: number) {
    setLoading(true);
    try {
      const itemsResponse = await getAdminItems({ q, priority, status: status as any, page: p, pageSize: 20 });
