...
import TodoItemRow from '@/pages/tasks/TodoItem';
...
import AddTaskModal, { NewTaskPayload } from '@/pages/tasks/AddTaskModal';
import Portal from '@/components/common/Portal';
import { PriorityBadge } from '@/pages/life-areas/Priority';
import { CategoryBadge } from '@/pages/life-areas/Category';
import { LifeAreaBadge } from '@/pages/life-areas/LifeArea';
import { PriorityKey, priorityOf } from '@/utils/priority';
import { CategoryKey, CHILD_CATEGORY_OF, categoryOf, goalLabelFor } from '@/utils/category';
import { GoalOption } from '@/services/api';
import { DynamicIcon } from '@/utils/icons';
import { sortItems } from '@/utils/organize';

const CONFETTI_COLORS = ['#a78bfa', '#e879f9', '#f0b429', '#34d399', '#38bdf8'];
...
export default function TodoList({
        ...
        {editModalOpen && (
          <AddTaskModal
            ...
            editTarget={{
              ...
              targetMonth: list.targetMonth ?? null,
              targetWeek: list.targetWeek ?? null,
              targetDayOfWeek: list.targetDayOfWeek ?? null,
  ...
  async function handleEditSave(id: string, data: NewTaskPayload) {
    const previousFields = {
      ...
      targetYear: list.targetYear ?? null,
      targetMonth: (list as any).targetMonth ?? null,
      targetWeek: (list as any).targetWeek ?? null,
      targetDayOfWeek: (list as any).targetDayOfWeek ?? null,
      lifeAreaId: list.lifeArea?.id ?? list.lifeAreaId ?? null,
      parentGoalId: list.parentGoal?.id ?? list.parentGoalId ?? null,
      startTime: list.startTime ?? null,
      endTime: list.endTime ?? null,
